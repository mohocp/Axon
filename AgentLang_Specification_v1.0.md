...
```
TYPE UserId = UInt64
TYPE Result[T] = SUCCESS(T) | FAILURE(ErrorCode, Str)
...
OPERATION calculate_discount =>
    ...
    OUTPUT discount: Float64 :: range(0.0..0.5)
    REQUIRE customer.status EQ active
    REQUIRE order.total GT 0
    ENSURE discount <= customer.tier.max_discount
    BODY {
...
MATCH result =>
    ...
    WHEN FAILURE(NOT_FOUND, msg) -> create_default_user()
    WHEN FAILURE(TIMEOUT, msg) -> RETRY(3, backoff: exponential)
    WHEN FAILURE(_, msg) -> ESCALATE(msg)
```
...
OPERATION transfer_funds =>
    ...
    OUTPUT receipt: TransferReceipt
    REQUIRE amount GT 0
    ...
    ENSURE receipt.status EQ COMPLETED
...
OPERATION validate_order =>
    ...
    BODY {
        ...
        WHEN stock_check.length GT 0
            -> EMIT FAILURE(OUT_OF_STOCK, stock_check)
        OTHERWISE
